...
class capesUI
{
	...
	public function mainForm(Player $player) {
        ...
        if (isset($data[0])){
				...
				if(strtolower($player->getSkin()->getSkinId()) === "standard_custom")
				{
	...
	private function getCapeCustomName() : array
	{
		$capes = array(
	...
	private function applyMaleCape(int $cape, Player $player) : void {
		switch($cape){
			case 1:
	...
	private function applyFemaleCape(int $cape, Player $player) : void {
		switch($cape){
			case 1:
